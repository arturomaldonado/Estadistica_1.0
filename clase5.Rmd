...
Nota: si es p-value es \> 0.05, la conclusión es que no se puede rechazar la Ho.
...
La diferencia de medias entre el número de alumnos con los que trabaja un profesor hombre y una profesora mujer es:

```{r dif alumnos por sexo}
mean(endo2020$P1_6[endo2020$P1_1==1], na.rm = T) - mean(endo2020$P1_6[endo2020$P1_1==2], na.rm = T)
```

¿Se puede afirmar que existe esta diferencia en la población?
...
```{r Levene}
library(DescTools)
LeveneTest(endo2020$P1_6, endo2020$P1_1)
```
...
```{r prueba t}
t.test(P1_6 ~ P1_1, data = endo2020, var.equal=F)
```
...
Esto se puede hacer mediante el comando `table`, pero en lugqr de describir una variable, se puede hacer la tabla cruzada.

```{r tabla nombrados x sexo}
table(endo2020$P1_7, endo2020$P1_1)
```

Con esta tabla se encuentra que entre los hombres (código 1 porque no se ha convertido en factor) se tiene 3303 éxitos y para las mujeres un total de 7140 éxitos.
...
```{r nombrado x sexo total}
addmargins(table(endo2020$P1_7, endo2020$P1_1))
```
...
```{r prop test 0}
prop.test(x=c(3303, 7140), n=c(6170, 12779), conf.level=0.95)
```
...
Se puede hacer lo mismo para evaluar la diferencia en la proporción de profesores que han recibido apoyo psicológico (P1_13) entre hombres y mujeres.
En primer lugar, se calcula la tabla cruzada.

```{r apoyo psi x sexo}
addmargins(table(endo2020$P1_13, endo2020$P1_1))
```

Luego se calcula la diferencia de proporciones.

```{r prop test}
prop.test(x=c(2777, 6699), n=c(6163, 12767), conf.level=0.95)
```

Se encuentra que el 45% de profesores hombres versus el 52.7% de profesoras mujeres han recibido apoyo psicológico.
Se encuentra que el p-value es \< 0.05 por lo que se rechaza la igualdad de proporciones y se afirma que son diferentes en la población.

Este resultado también se puede calcular de frente anidando la tabla en el comando.
Este procedimiento no da el mismo valor de p-value, pero no nos ofrece las proporciones correctas.

```{r anidado}
prop.test(table(endo2020$P1_13, endo2020$P1_1))
```
...
En la sección anterior se evaluó de manera "informal" o "visual" la hipótesis de Lalonde acerca de la diferencia en ingresos entre aquellos que son parte del programa de capacitación y aquellos que no son parte del programa, y entre aquellos que son afro y los que no.
...
¿Son esos 880 dólares de diferencia extrapolables a la población?
Es decir, a partir de esta diferencia muestral, ¿se puede decir que existe una diferencia de ingresos entre la población que formó parte del programa y aquellos que no formaron parte?
...
Como el p-value = 0.14 es \> 0.05, se falla en rechazar la igualdad de varianzas, por lo que se tiene que asumir que son iguales.
Para evaluar si existen o no diferencias poblacionales, usamos el comando `t.test`.
Este comando requiere de la especificación `var.equal=T`, de acuerdo a los resultados de la prueba de Levene.
...
Sin embargo, si se trabajara con un límite más permisible de 0.10, entonces sí se podría rechazar la H0 y afirmar que las diferencias son estadísticamente significativas, y, por lo tanto, extrapolables a la población, aunque con una mayor probabilidad de un error de tipo 1, que se verá más adelante.

Si se quisiera evaluar las diferencias de ingresos entre afros y no afros, lo primero es recordar las diferencias.

```{r afro}
mean(LL$re78[LL$black==0]) - mean(LL$re78[LL$black==1])
```
...
Nuevamente, p-value = 0.57 \> 0.05, entonces no se puede rechazar la H0 de igualdad de varianzas.
Luego, se corre la prueba t.

```{r prueba t afro}
t.test(re78 ~ black, data = LL, var.equal=T)
```

En este caso, el p-value = 0.0054 \< 0.05, por lo que se rechaza la H0 y se concluye que la diferencia de 1,600 dólares entre afroamericanos y no afroamericanos sí existe en la población.

# Ejemplo de diferencia de proporciones

En la sección anterior se hizo referencia al texto de Galarza, Yamada y Kogan [@galarzaDetectandoDiscriminacionSexual2012].
Para este ejemplo vamos a usar el paper original [@bertrandAreEmilyGreg2004] que buscan evaluar si hay una discriminación en el mercado laboral entre afroamericanos y angloamericanos.
...
-   firstname: nombre de postulante ficticio.
    Esta variable es de tipo "chr", es decir de caracteres.
...
-   call: si el postulante recibió una llamada (1=sí, 0=no).
    Esta variable es de tipo "int", es decir numérica de números enteros.
...
Se tiene que tomar en cuenta que la variable "race" es importada como una variable "chr" que indica que no es numérica, sino una cadena de caracteres, por lo que la selección se tiene que hacer usando las comillas, mediante `[cv$race=="white"]`.
Si se calculara una variable nueva "raza" como tipo numérico, entonces la selección se haría con la especificación `[cv$raza==1]`.
SI se transformara esta variable "raza" en una de tipo factor, entonces la selección se haría con la especificación `[cv$raza=="Blanco"]`.

Lo mismo se puede hacer con la variable "call" que es importada como una variable "int" , que es numérica con enteros.
Sin embargo, esta variable es también un factor, por lo que se puede crear una nueva variable "llamada" de tipo factor.
...
Una primera opción, aunque **referencial**, es usar la prueba t, dado que si la proporción se puede calcular mediante la media, la diferencia de proporciones se podría evaluar mediante la diferencia de medias.
...
```{r prueba t 2, message=TRUE, warning=TRUE}
...
OJO: si se quisiera hacer esta evaluación de diferencia de proporciones mediante la prueba t usando las variables transformadas de factor, la prueba no correría pues exige una variable numérica como variable dependiente.
Con una variable factor como dependiente se obtiene un error.

```{r prueba t 3, eval=FALSE, include=FALSE}
t.test(llamada ~ raza, data = cv, var.equal=F)
```
...
De esta forma se calcula el mismo p-value y el mismo intervalo de confianza de la diferencia de proporciones.
Sin embargo, de esta manera no se obtiene las proporciones adecuadas.
...
O, se puede calcular de frente anidando la tabla en el comando.
