...
-   Luego, plantear la distribución muestral. Como el error estándar depende de si las varianzas parecen ser iguales o no, tenemos que correr una prueba para definir esto. Esta prueba se llama tests de Levene. Este test está disponible en la librería `Desctools`, con el comando `LeveneTest`.
...
-   Para evitar falsos positivos se propone pedir evidencia cada vez más sólida para condenar a alguien.
    Pero, si se exige evidencia cada vez más solida, aumenta la probabilidad de falsos negativos.
    Ambos tipos de errores se contrabalancean mutuamente.
