...
En esta sección vamos a trabajar con la información de las elecciones 2021.
Los resultados que usaremos corresponden al conteo rápido elaborado por [IPSOS](https://www.ipsos.com/es-pe/resultados-del-conteo-rapido-al-100-6-de-junio-2021) y a datos sociodemográficos, disponibles en la web del [INEI](https://www.inei.gob.pe/).

```{r base2, message=FALSE, warning=FALSE}
library(rio)
data21 <- import("https://raw.github.com/arturomaldonado/Estadistica_1.0/main/voto2021.xlsx")
```

En esta oportunidad, usaremos el voto a Pedro Castillo según conteo rápido por departamento como variable dependiente y el ingreso mensual medido en 2020 como variable independiente.
Se parte de la hipótesis que existiría una relación inversa entre estas variables.
Para hacer una primera verificación de esta relación, se puede crear el diagrama de dispersión.

```{r dispersion21}
plot(data21$VotoPL, data21$IngresoMensual2020,
     xlab="Ingreso mensual promedio 2020",
     ylab="Voto a Pedro Castillo (conteo rápido 2da vuelta)",
     pch=15, xlim=c(400,1400), ylim=c(20, 100))
text(data21$IngresoMensual2020, data21$VotoPL, labels=data21$Departamento, cex=0.6, pos=4)
```

El gráfico no nos muestra una relación muy clara entre las variables.
Para analizar en más detalle, más allá de la relación visual, analizaremos el modelo de regresión lineal.
Como indicamos antes, el modelo se calcula con el comando `lm` donde se indica la variable Y y luego la X.
Este modelo de guarda en un objeto "modelo2" el que se puede describir con el comando `summary`.

```{r modelo2}
modelo2 <- lm(data21$VotoPL ~ data21$IngresoMensual2020)
summary(modelo2)
```
...
Es decir, se pone a prueba cuál es la probabilidad de observar una pendiente de -0.03 si la "verdadera" pendiente es cero.
Esta probabilidad se mide, como siempre, con el p-value (o Pr(\>\|t\|), en la tabla de resultados).

En la fila correspondiente a la variable independiente se observa que el p-value = 0.123 (o 12.3%).
Es decir, que tenemos 12.3% de probabilidades de observar una pendiente de -0.03 si la verdadera pendiente es cero.
Como este valor es mayor a 0.05, NO se puede rechazar la H0.

Con esto se concluye que no se puede decir que haya relación entre ambas variables.

En un segundo ejemplo, analizaremos la relación entre el analfabetismo en un departamento y el voto a Pedro Castillo.
Guardaremos el modelo lineal en un nuevo objeto "modelo3" que describiremos.

```{r}
modelo3 <- lm(data21$VotoPL ~ data21$Analfabetismo)
summary(modelo3)
```

La ecuación de la recta, según estos resultados es:

$$\hat{Y} = 40.59 + 2.93*X$$

El valor de $\hat{\beta} = 2.93$, con un p-value = 0.00168 (o 0.168%).
Como el p-value es menor a 0.05 (o 5%), entonces podemos decir que la probabilidad de observar una pendiente de 2.93, siendo la verdadera cero es muy baja, por lo que lo más probable es que la pendiente no sea cero.

Se rechaza la H0 y se afirma la HA, concluyendo que sí existe relación entre las variables.
...
En nuestro ejemplo, por cada 1% de aumento en la tasa de analfabetismo, el voto a Pedro Castillo cambia 2.9%.

Pero la pendiente no es un buen indicador de la fuerza de la relación entre las variables, pues depende de las unidades de medida de las variables.
...
En nuestro ejemplo r=0.596.
Es decir, por cada unidad de desviación estándar de cambio en X, Y cambia 0.596 desviaciones estándar.

```{r correlacion}
cor.test(x=data21$Analfabetismo, y=data21$VotoPL, method="pearson")
```
...
En el contexto del análisis de regresión lineal, la fuerza de la relación entre dos variables se interpreta como qué tan bien X predice Y.
Esta capacidad de X de predecir la variable Y se mida mediante el coeficiente de determinación $R^2$.
...
En nuestro ejemplo, $R^2=0.35$.
Este valor se encuentra en la penúltima fila del `summary` del modelo, con el nombre "Multiple R-squared".
Es decir, el modelo reduce un 35% el error de usar solamente el promedio para estimar Y.
Es decir, en el rango entre 0 y 1, el valor de 0.35 corresponde a una fuerza moderada entre las variables.
