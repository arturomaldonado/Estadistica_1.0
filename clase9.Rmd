...
Se obtiene que la suma total de residuos al cuadrado es 3231.8.
...
Una forma de "predecir" el voto a PL si no se tuviera información sería solo mediante el promedio del voto a PL.
El voto promedio a PL es 27.3%.
Sin otra información esta sería la mejor aproximación.

```{r media}
mean(data$VotoPL_1, na.rm=T)
```

Esta aproximación se puede graficar con una recta horizontal Y=27.3.
...
```{r}
data$res2 = data$VotoPL_1-27.28
sum(data$res2^2, na.rm=T)
```

Se observa que la suma de residuos de la recta calculada por R tiene una suma de residuos al cuadrado (3231.8) menor que la suma de residuos al cuadrado con respecto a la recta horizontal (6086.4).
En realidad, se podría hacer este ejercicio con cualquier recta y se obtendría una suma de residuos al cuadrado mayor de 3231.8.
...
Hemos usado el voto a Pedro Castillo en primera vuelta por departamento como variable dependiente y el porcentaje de personas analfabetas como variable independiente.
...
ggplot(data, aes(x=Analfabetismo, y=VotoPL_1))+
  geom_point()+
  geom_smooth(method=lm, se=F)+ #agregar línea de tendencia
  geom_text(data=data, aes(label=Departamento), cex=2.5, nudge_y = 0.6, check_overlap = T)+ #Pata etiquetar los puntos, darles un tamalo, ubicación y prevenir que se sobrepongan
  labs(x="Porcentaje de Analfabetismo", 
       y="Voto a Castillo (1era vuelta)")+ #para etiquetar los ejes
  theme_light()
```

El gráfico nos muestra una relación clara entre las variables.
...
Este modelo de guardó en un objeto "modelo1" el que se puede describir con el comando `summary`.
