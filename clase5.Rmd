...
Queremkos evaluar la diferencia de medias entre el número de alumnos con los que trabaja un profesor hombre y una profesora mujer.
...
tabla1 = endo2020 |>
  ...
  group_by(sexo) |>
  dplyr::count(apoyo = apoyo, name="N")|>
  mutate(total = sum(N))
tabla1
```
...
```{r}
endo2020 = endo2020 |>
  mutate(condicion = factor(P1_7, labels=c("Nombrado", "Contratado por concurso", 
...
tabla2 = endo2020 |>
  ...
  group_by(sexo) |>
  dplyr::count(condicion = condicion, name="N")|>
  mutate(total = sum(N))
tabla2
```
