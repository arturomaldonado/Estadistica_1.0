...
También se pouede calcular la mediana, con el comando `median`.
...
También se puede calcular la media para grupos de observaciones, es decir para files específicas de una base de datos.
Por ejemplo, si quisiéramos saber si el voto en las provincias de Lima fue mayor o menos que el voto provincial general de Fuerza Popular, podríamos calcular la media del voto provincial solo en las provincias de Lima.

Esto lo hacemos usando los [..], donde anidamos una condición, que en este caso se hace con los valores de otra variable "dpto" para seleccionar solo los datos de Lima.
Para hacer este tipo de operaciones se tiene que usar los operadores lógicos.
...
```{r}
mean(res2016$fp[res2016$dpto =="TUMBES"])
```
...
Para ver esta asimetría se puede graficar la distribución de frecuencias de esta variable.
...
Podemos usar el comando `table` para calcular la tabla de distribución de frecuencias de esta variable.
...
Para calcular los porcentajes, se puede anidar el comando anterior en otro comando `prop.table()` que nos calcula las proporciones (entre 0 y 1).
Para calcular los porcentajes se multiplica este comando por 100.
...
La otra es usar una librería `ggplot2` que permite una mayor flexibilización en la producción y estética de los gráficos.
...
Esto lo comprobamos produciendo el histrograma de esta variable.
...
```{r}
library(car)
res2016$region = car::recode(res2016$dpto, "'AMAZONAS'=3; 'ANCASH'=2; 'APURIMAC'=2;
                                            'AREQUIPA'=2; 'AYACUCHO'=2; 'CAJAMARCA'=2;
                                            'CUSCO'=2; 'CALLAO'=1; 'HUANCAVELICA'=2;
                                            'HUANUCO'=3; 'ICA'=1; 'JUNIN'=2;
                                            'LA LIBERTAD'=1; 'LAMBAYEQUE'=1;
                                            'LIMA'=1; 'LORETO'=3; 'MADRE DE DIOS'=3;
                                            'MOQUEGUA'=1; 'PASCO'=2; 'PIURA'=1;
                                            'PUNO'=2; 'SAN MARTIN'=3; 'TACNA'=1;
                                            'TUMBES'=1; 'UCAYALI'=3")
table(res2016$region)
```

La variable creada es una variable numérica.
En R existe otro tipo de variable llamado "factor".
Podemos convertir esta variable numérica a una de factor y etiquetar cada valor.
...
El gráfico de cajas también muestra una medida de hetorogeneidad: la amplitud de la caja o llamado rango intercuartil.
...
En R se puede calcular con el comando `IQR`.
...
```{r}
sd(res2016$fp2011[res2016$region==1])
sd(res2016$fp2011[res2016$region==2])
sd(res2016$fp2011[res2016$region==3])
```
