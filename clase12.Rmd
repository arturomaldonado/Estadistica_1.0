...
library(tidyverse)
data16 = data16 |>
  mutate(pbigrupo = factor(pbipc_grupos, labels=c("Bajo", "Medio", 
                                                  "Alto")))
```
...
```{r ols multiple}
modelo3 = lm(Voto_PPK_2016_1 ~ Uso_internet + pbipc_2014, data=data16)
summary(modelo3)
```
...
El estadístico F = 7.71 tiene un p-value asociado de 0.0012.
...
Para la variable uso de internet, la fila correspondiente nos indica que tenemos un coeficiente de 0.185.
...
Es decir, se tiene 18.6% de probabilidades de tener un coeficiente de -0.06, si el verdadero valor del parámetro es 0.
...
En nuestro ejemplo se tiene un $R^2=0.37$.
Esta dato indicaría que el modelo que incluye las variables acceso a internet y PBI per cápita explican el 37% de la variación total del voto a PPK.
