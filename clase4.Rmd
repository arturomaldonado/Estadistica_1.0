...
También podemos usar la base de datos de la Encuesta Nacional Docente 2020.
...
Con esta base se pueden calcular los intervalos de confianza del número de alumnos que acompañan los docentes hombres y mujeres.
...
-   El IC al 95% de confianza del número de alumnos para los docentes hombres es \[50.7 - 53.8\]

-   El IC al 95% de confianza del número de alumnos para las docentes mujeres es \[33.3 - 34.8\]

Como ambos intervalos no se cruzan, podemos afirmar preliminarmente que los docentes hombres atienden a más alumnos en aula en promedio que las docentes mujeres en el Perú.
...
De la misma manera, para graficas el número de alumnos que atienden los docentes hombre y mujeres, lo primero es crear una tabla con los resultados y guardarla en un objeto.

```{r}
alum <- group.CI(P1_6~P1_1, endo2020)
alum
```

Luego, usamos este objeto para crear el gráfico con la librería `ggplot`.

```{r grafico ggplot docentes}
graf2 <- ggplot(alum, aes(x=P1_1, y=P1_6.mean))+
  geom_bar(width=0.5, fill="darkcyan", colour="black", stat="identity")+
  geom_errorbar(aes(ymin=P1_6.lower, ymax=P1_6.upper), width=0.2)+
  geom_text(aes(label=paste(round(P1_6.mean, 1))), vjust=-2.5, size=4)+
  xlab("Género") + ylab("Alumnos")+
  ylim(0, 60)
graf2
```

¿Por qué la variable género presenta los valores 1.0 y 2.0, con 1.5 en medio?
